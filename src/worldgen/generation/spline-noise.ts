...
export class SplineNoiseGenerator implements IWorldGenerator {
  ...
  getDefaultParams(): Record<string, number> {
    return {
      continentalFreq: 0.006, erosionFreq: 0.015, peaksFreq: 0.04,
      continentalWeight: 0.5, erosionWeight: 0.3, peaksWeight: 0.2,
      baseHeight: 10, heightScale: 50,
    }
  }
  ...
  generate(config: GenerationConfig): GenerationResult {
    ...
    for (let x = 0; x < worldWidth; x++) {
      for (let z = 0; z < worldDepth; z++) {
        ...
        const combined = (cVal * params.continentalWeight + eVal * params.erosionWeight + pVal * params.peaksWeight) / totalWeight
        const splineVal = evaluateSpline(spline, combined)
        const clampedHeight = Math.max(1, Math.min(worldHeight - 1, Math.floor(params.baseHeight + splineVal * params.heightScale)))
